...
const ColorChanger: FC<ColorChangerProps> = ({ slice }) => {
  ...
  return (
    ...
    >
      ...
      >
        <text
          className="motion-sage:transition-all fill-white/20 font-semibold uppercase italic group-hover:fill-white/30 motion-safe:duration-700"
          fontSize={7}
          textAnchor="middle"
      ...
      <Canvas camera={{ position: [0, 0.4, 0.3], fov: 45, zoom: 1.5 }}>
        <Scene
          selectedTextureId={selectedTextureId}
          onAnimatinoComplete={handleAnimationComplete}
        />
      </Canvas>
      ...
      >
        <div className="max-w-md shrink-0">
          <h2 className="mb-1 text-4xl font-semibold uppercase italic lg:mb-2 lg:text-6xl">
        ...
        <ul className="grid grow grid-cols-2 gap-3 rounded-2xl bg-white p-4 text-black shadow-lg sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-3 xl:grid-cols-6">
          {KEYCAP_TEXTURES.map((texture) => (
            <li key={texture.id}>
              <button
                className={clsx(
                  "flex aspect-square flex-col items-center justify-center rounded-lg border-2 p-4 hover:scale-104 motion-safe:transition-all motion-safe:duration-300",
