    ...
    >
      <FadeIn>
        <h2
          ...
          className="scroll-pt-6 text-6xl font-semibold uppercase italic md:text-8xl"
        >
...
const SharedCanvas = ({ color }: SharedCanvasProps) => {
  ...
  return (
    <div className="group relative min-h-96 overflow-hidden rounded-3xl select-none">
      ...
      <button
        onClick={handleSound}
        className="absolute bottom-0 left-0 z-10 flex items-center gap-3 p-6 text-4xl font-black text-white uppercase italic focus:ring-2 focus:ring-white focus:outline-none"
      >
        {name} <LuVolume2 />
      </button>
      {/* canvas */}
      ...
      >
        <svg className="pointer-events-none h-auto w-full" viewBox="0 0 75 100">
          <text
            ...
            textAnchor="middle"
            fontSize={18}
            className="font-black-slanted fill-white/30 uppercase mix-blend-overlay group-hover:fill-white/70 motion-safe:transition-all motion-safe:duration-700"
          >
